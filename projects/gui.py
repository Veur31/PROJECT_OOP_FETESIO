...
def login():
    ...
    if login_result:
        ...
        for content in content_contain.winfo_children():
            content.destroy()
        Label(content_contain, text=f"Welcome Admin, \n{login_result.getName()}!", font=("Century Gothic", 28), fg="#00BFFF", bg="#1e1d1d").grid(row=0, column=0, sticky="ew", pady=350, padx=100)
    else:
        error.config(text="Invalid id number", fg="#e17676", font=("Roboto", 20))
        error.pack(pady=10)
...
def view_information():
    ...
    if curr_stu: 
        ...
        Label(content_contain, text="Information\n", font=("Century Gothic", 25, "bold"), fg="#00BFFF", bg="#1e1d1d").grid(row=1, column=0, columnspan=2,  pady=10, padx=100)
        Label(content_contain, text=curr_stu, font=("Century Gothic", 18), fg="#00BFFF", bg="#1e1d1d", justify="left").grid(row=2, column=0, sticky="ew", pady=1, padx=150)
    else:
...
def view_other_information():
    ...
    Label(content_contain, text="Enter Id", font=("Century Gothic", 25, "bold"),  bg="#1e1d1d", fg="#00BFFF").grid(row = 1, column= 0, columnspan =2, pady= 50, padx=150)
...
Label(float_frame, text="Welcome to Student Management System \n \nEnter your ID", font=("Roboto", 25),fg="#00BFFF",padx=20,bg="#1e1d1d").pack()
login_btn = Button(float_frame, text="Login", width=10, font=("Roboto", 18),bg="#00BFFF", command=login)

login_entry = Entry(float_frame, width=20, font= ("Century Gothic", 20), show="*")
...
Label(content_contain, text="Content", font=("Roboto", 20),fg="white", bg = 'black', padx=20).grid(row=0, column=0, sticky="ew") 
for i, txt in enumerate(btn_txt1):
    btns = Button(content_contain, anchor="center", width=40, text=btn_txt1[i], font=("Roboto", 14), padx=10, pady=15, bg="#FFFFFF")
    btns.grid(row=i+1, column=0)
    btns.config(command=partial ())
logout_btn = Button(menu_contain, text="Logout", width=20, font=("Roboto", 20),padx=10,pady=20, command=logout)
